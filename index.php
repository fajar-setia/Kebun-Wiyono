...
$redirect_uri = 'http://localhost/MPTI/tugasMPTI/index.php';
...
if (isset($_GET['code'])) {
    ...
    if (!isset($token['error'])) {
        ...
        $query_check = 'SELECT * FROM pengguna WHERE google_id = ?';
        ...
        if ($user = $result->fetch_assoc()) {
            ...
            $query_update = 'UPDATE pengguna SET nama_lengkap = ?, email = ? WHERE google_id = ?';
            $stmt_update = $conn->prepare($query_update);
            $stmt_update->bind_param("sss", $g_name, $g_email, $g_id);
            
            if (!$stmt_update->execute()) {
                $loginError = "Tidak berhasil login";
                echo "<script>alert('hai , " . $loginError . "!');</script>";
            } else {
                // PERBAIKAN: Simpan user ID yang sebenarnya
            ...
            // Insert new user
            $query_insert = 'INSERT INTO pengguna (nama_lengkap, email, google_id) VALUES (?, ?, ?)';
            ...
            if (!$stmt_insert->execute()) {
                ...
                // PERBAIKAN: Ambil ID user yang baru saja dibuat
